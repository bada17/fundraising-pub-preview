...
- ⚠️ **HTTPS 인증서가 `bad_authz` 로 떨어지면, 고치는 길은 하나다** —
  **Settings → Pages 에서 Custom domain 을 비우고 Save, 다시 넣고 Save.**
  `CNAME` 파일을 지웠다 되돌리는 것도, API 로 같은 `cname` 을 다시 PUT 하는 것도
  안 통했다(2026-09-16 에 둘 다 해 봤다). "DNS check successful" 이 뜨면 발급된다.
  ⚠️ 설정에서 비우면 **깃허브가 `docs/CNAME` 을 지우는 커밋을 만든다** — 그 뒤에는
  반드시 `git pull` 부터 한다.
- ⚠️ 후보 이름을 고를 때 **공개 DNS 에 미리 조회하지 말 것.** 와일드카드 때문에
  "없음" 이 아니라 아임웹 주소가 답으로 오고 TTL 1시간으로 캐시에 박힌다 —
  레코드를 넣은 뒤에도 옛 답을 주는 구간이 생겨 검증이 거기 걸린다.
